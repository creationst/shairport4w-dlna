 ...
 *
 *  This file is originally part of libao, a cross-platform audio output library.  See
    ...
    if(device->verbose==2){                                             \
      if(strcmp(format,"\n")){                                          \
        if(device->funcs->driver_info()->short_name){                   \
          ATLTRACE("ao_%s debug: " format,device->funcs->driver_info()->short_name,## args); \
        }else{                                                          \
          ATLTRACE("debug: " format,## args);                     \
        }                                                               \
      }else{                                                            \
        ATLTRACE("\n");                                           \
      }                                                                 \
...
#define averbose(format, args, ...) {\
    if(device->verbose>0){                                              \
      if(strcmp(format,"\n")){                                          \
        if(device->funcs->driver_info()->short_name){                   \
          ATLTRACE("ao_%s info: " format,device->funcs->driver_info()->short_name,## args); \
        }else{                                                          \
          ATLTRACE("info: " format,## args);                      \
        }                                                               \
      }else{                                                            \
        ATLTRACE("\n");                                           \
      }                                                                 \
...
#define awarn(format, args, ...) {\
    if(device->verbose>=0){                                             \
      if(strcmp(format,"\n")){                                          \
        if(device->funcs->driver_info()->short_name){                   \
          ATLTRACE("ao_%s WARNING: " format,device->funcs->driver_info()->short_name,## args); \
        }else{                                                          \
          ATLTRACE("WARNING: " format,## args);                   \
        }                                                               \
      }else{                                                            \
        ATLTRACE("\n");                                           \
      }                                                                 \
...
#define aerror(format, args, ...) {                                       \
    if(device->verbose>=0){                                             \
      if(strcmp(format,"\n")){                                          \
        if(device->funcs->driver_info()->short_name){                   \
          ATLTRACE("ao_%s ERROR: " format,device->funcs->driver_info()->short_name,## args); \
        }else{                                                          \
          ATLTRACE("ERROR: " format,## args);                     \
        }                                                               \
      }else{                                                            \
        ATLTRACE("\n");                                           \
      }                                                                 \
